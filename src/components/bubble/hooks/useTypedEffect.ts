...
const useTypedEffect = (
...
): [typedContent: React.ReactNode | object, shouldContinueTyping: boolean] => {
  ...
  const mergedTypingEnabled = isTyping && isString(content);
  ...
  }, [typingIndex, isTyping, content]);

  //应该实时显示的内容
  const mergedTypingContent = mergedTypingEnabled ? content.slice(0, typingIndex) : content;
  //是否应该继续打字
  const shouldContinueTyping = mergedTypingEnabled && typingIndex < content.length;
